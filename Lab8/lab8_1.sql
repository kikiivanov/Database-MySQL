...
VALUES (1, month(now()), year(now()), 1200, now()),
	   ...
	   (3, month(now()), year(now()), 1500, now());

#1
CREATE VIEW studentView
AS 
SELECT coaches.name AS coachName, concat(sportgroups.id , " - " , sportgroups.location), sports.name as sportName,  year(now()), month(now()), salarypayments.salaryAmount
FROM coaches 
JOIN sportgroups
